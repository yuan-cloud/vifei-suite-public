use panopticon_tour::{TourConfig, TourStageProfile};
...
fn main() {
    let iters = std::env::var("PANOPTICON_TOUR_PROFILE_ITERS")
        .ok()
    ...
    for _ in 0..iters {
        ...
        let (_, profile): (_, TourStageProfile) =
            panopticon_tour::run_tour_with_profile(&config).expect("tour run");
