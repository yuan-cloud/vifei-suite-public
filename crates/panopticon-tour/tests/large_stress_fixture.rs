...
use panopticon_tour::{TourConfig, TourMetrics};
...
fn tour_processes_large_fixture() {
    ...
    let result = panopticon_tour::run_tour(&config).expect("Tour must process large fixture");
...
fn tour_large_fixture_deterministic() {
    ...
    let r1 = panopticon_tour::run_tour(&config1).unwrap();
    let r2 = panopticon_tour::run_tour(&config2).unwrap();
