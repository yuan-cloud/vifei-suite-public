...
cargo test -p panopticon-import --test adapter_conformance_drift -- --nocapture \
  2>&1 | tee "$out_dir/adapter-conformance-drift.log"

echo "Adapter conformance drift gate passed."
echo "Replay command: cargo test -p panopticon-import --test adapter_conformance_drift -- --nocapture"
